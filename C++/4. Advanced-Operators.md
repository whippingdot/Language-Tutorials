...
**Note: In English, normally when you say that you're going to a store ___or___ a concert, you mean that you are intending to go to only one of the places, not neither, not both; C++'s `||` operator returns `true` for `true || true`, doesn't provide an operator that would yield `false` for `true || true`. (there is a way to capture the former meaning though, and will be covered later)**
